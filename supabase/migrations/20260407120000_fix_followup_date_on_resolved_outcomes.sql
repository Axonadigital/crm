-- Migration: Fix next_followup_date not being cleared on resolved outcomes
-- Created: 2026-04-07
-- Description: When a call is logged with outcome meeting_booked, not_interested, or bad_fit
--              without an explicit followup_date, the old next_followup_date was never cleared.
--              This caused companies with booked meetings to keep appearing in the follow-up list.
--              Fix: clear next_followup_date (and related fields) for resolved outcomes when no
...
BEGIN
  ...
  SET
    ...
    -- Explicit date → set it. Resolved outcome without date → clear it. Otherwise → keep.
    next_followup_date = CASE
      WHEN v_followup_ts IS NOT NULL THEN v_followup_ts
      WHEN p_call_outcome IN ('meeting_booked', 'not_interested', 'bad_fit') THEN NULL
    ...
    pipeline_state = CASE
      ...
      WHEN pipeline_state IS NULL AND v_new_lead_status = 'lost' THEN 'lost'
      ELSE pipeline_state
    END
  WHERE id = p_company_id;
